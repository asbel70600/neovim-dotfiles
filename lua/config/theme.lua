...
local function random_theme(themes, setup_function)
    ...
    local success, err = pcall(function()
        ...
        if selected_theme.setup ~= "" then
            require(selected_theme.setup).setup({ style = "auto" })
        end
...
vim.api.nvim_create_user_command("ColorschemeDarkRandom", function()
    random_theme(dark_themes, function()
        vim.opt.background = "dark"
    end)
end, {})

vim.api.nvim_create_user_command("ColorschemeLightRandom", function()
